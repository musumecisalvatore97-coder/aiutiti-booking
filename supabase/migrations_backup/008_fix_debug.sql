...
BEGIN
    ...
    -- 2. Lookup (Simplified)
    -- Match ANY record with same session_id or chat_id
    ...
    IF p_party_size IS NOT NULL AND p_start_at IS NOT NULL THEN
        BEGIN
            ...
            v_new_option_id := v_found_join_id; -- can be null
        EXCEPTION WHEN OTHERS THEN
    ...
    ELSE
        ...
        ELSE
             ...
             -- Get a sample row safely
             SELECT chat_id, session_id, source INTO v_dbg_chat, v_dbg_sess, v_dbg_src
             FROM pending_reservations LIMIT 1;
             
             RAISE EXCEPTION 'DEBUG FAIL: No Match. Input: chat=% sess=% source=%. Total Rows: %. Sample: chat=% sess=% src=%', 
