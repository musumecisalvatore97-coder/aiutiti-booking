...
BEGIN
    ...
    ELSE
        ...
        ELSE
             -- DEBUG FAILURE: EXTENDED PARITY CHECK
             SELECT count(*) INTO v_tot FROM pending_reservations;
             
             SELECT pending_id, chat_id, session_id, source
             INTO v_last_id, v_last_chat, v_last_sess, v_last_src
             FROM pending_reservations 
             ORDER BY created_at DESC 
             LIMIT 1;
             
             -- Compare MD5 and Lengths
             RAISE EXCEPTION 'DEBUG PARITY: 
Input Chat: "%" (Len: %, MD5: %)
DB Last Chat: "%" (Len: %, MD5: %)
Input Sess: "%" (Len: %, MD5: %)
DB Last Sess: "%" (Len: %, MD5: %)
Match Chat? %
Match Sess? %
ID: %', 
                v_safe_chat_id, length(v_safe_chat_id), md5(v_safe_chat_id),
                v_last_chat, length(v_last_chat), md5(v_last_chat),
                v_safe_session_id, length(v_safe_session_id), md5(v_safe_session_id),
                v_last_sess, length(v_last_sess), md5(v_last_sess),
                (v_safe_chat_id = v_last_chat),
                (v_safe_session_id = v_last_sess),
                v_last_id;
        END IF;
    END IF;
