-- 023_liveops_enhancements_types_fix.sql

-- Drop the incorrectly typed functions first
DROP FUNCTION IF EXISTS close_table_session(text, bigint, numeric);
DROP FUNCTION IF EXISTS get_todays_reservations(bigint);
...
CREATE OR REPLACE FUNCTION get_todays_reservations(p_tenant_id uuid)
RETURNS TABLE (
    reservation_id uuid,
    customer_name text,
    phone text,
