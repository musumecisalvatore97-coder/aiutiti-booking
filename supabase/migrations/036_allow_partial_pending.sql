-- 036_allow_partial_pending.sql
-- Fixes the state machine to allow partial reservations (e.g. knowing the date but not the party size) 
-- without returning NULL from web_upsert_pending.
...
BEGIN
    ...
    ELSE
        ...
        IF p_party_size IS NOT NULL OR p_start_at IS NOT NULL OR p_customer_name IS NOT NULL OR p_phone IS NOT NULL THEN
